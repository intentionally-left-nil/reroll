...
class TestName:
    ...
    def test_bare_name_with_no_mapper_opinion_falls_back_to_normalized_name(self) -> None:
        assert pep508_to_matchspec("Requests", (aggregator_mapper,)) == "requests"

    def test_bare_name_with_no_mapper_opinion_normalizes_separators_too(self) -> None:
        assert pep508_to_matchspec("Foo_Bar.BAZ", (aggregator_mapper,)) == "foo-bar-baz"
...
class TestOperators:
    ...
    def test_arbitrary_equality_is_converted_to_double_equals(self) -> None:
        assert pep508_to_matchspec("requests===2.0.0", (aggregator_mapper,)) == "requests ==2.0.0"

    def test_arbitrary_equality_against_a_non_pep440_string_passes_through(self) -> None:
        assert (
            pep508_to_matchspec("requests===some-weird-string", (aggregator_mapper,))
    ...
    def test_multiple_specifiers_with_the_same_operator_sort_lexicographically(self) -> None:
        ...
        """
        assert (
            pep508_to_matchspec("requests>=9.0,>=10.0", (aggregator_mapper,))
            == "requests >=10.0,>=9.0"
        )
    ...
    def test_equals_glob_is_rewritten_to_the_canonical_fuzzy_form(
        ...
        assert pep508_to_matchspec(entry, (aggregator_mapper,)) == expected

    def test_equals_glob_with_an_epoch_is_rewritten_to_the_canonical_fuzzy_form(self) -> None:
        assert pep508_to_matchspec("requests==1!2.0.*", (aggregator_mapper,)) == "requests =1!2.0"
    ...
    def test_not_equals_glob_passes_through_unchanged(self, entry: str) -> None:
        ...
        assert pep508_to_matchspec(entry, (aggregator_mapper,)) == f"{name} !={version}"

    def test_not_equals_glob_with_an_epoch_passes_through_unchanged(self) -> None:
        assert (
            pep508_to_matchspec("requests!=1!2.0.*", (aggregator_mapper,)) == "requests !=1!2.0.*"
        )

    def test_compatible_release_expands_to_a_range(self) -> None:
        assert (
            pep508_to_matchspec("requests~=3.13.2", (aggregator_mapper,))
            == "requests >=3.13.2,<3.14.0a0"
        )

    def test_compatible_release_with_two_segments_drops_the_major(self) -> None:
        assert (
            pep508_to_matchspec("requests~=3.13", (aggregator_mapper,)) == "requests >=3.13,<4.0a0"
        )
    ...
    def test_compatible_release_preserves_the_epoch_in_both_bounds(self) -> None:
        assert (
            pep508_to_matchspec("requests~=1!3.13.2", (aggregator_mapper,))
    ...
    def test_compatible_release_allow_pre_permits_a_pre_release(self) -> None:
        assert (
            pep508_to_matchspec("requests~=3.13.2rc1", (aggregator_mapper,), allow_pre=True)
            == "requests >=3.13.2.rc1,<3.14.0a0"
        )

    def test_version_with_a_v_prefix_is_normalized_away(self) -> None:
        assert pep508_to_matchspec("requests>=v1.0", (aggregator_mapper,)) == "requests >=1.0"
    ...
    def test_arbitrary_equality_against_a_glob_like_string_is_not_rewritten(self) -> None:
        ...
        """
        assert pep508_to_matchspec("requests===1.0.*", (aggregator_mapper,)) == "requests ==1.0.*"
    ...
    def test_arbitrary_equality_allow_pre_permits_a_pre_release_version(self) -> None:
        assert (
            pep508_to_matchspec("requests===1.0.0rc1", (aggregator_mapper,), allow_pre=True)
            == "requests ==1.0.0.rc1"
        )
...
class TestVersionFormatting:
    def test_epoch_is_preserved(self) -> None:
        assert (
            pep508_to_matchspec("requests>=1!1.0.0", (aggregator_mapper,)) == "requests >=1!1.0.0"
        )

    def test_post_release_is_accepted(self) -> None:
        assert (
            pep508_to_matchspec("requests>=1.0.0.post1", (aggregator_mapper,))
            == "requests >=1.0.0.post1"
        )

    def test_post_release_shorthand_is_normalized(self) -> None:
        assert (
            pep508_to_matchspec("requests>=1.0-1", (aggregator_mapper,)) == "requests >=1.0.post1"
        )
    ...
    def test_pre_post_and_dev_releases_all_combine_in_order(self) -> None:
        assert (
            pep508_to_matchspec(
                "requests==1.0.0a1.post2.dev3", (aggregator_mapper,), allow_pre=True
            )
...
class TestExtras:
    def test_bare_extra_becomes_an_extras_bracket(self) -> None:
        assert pep508_to_matchspec("fastapi[all]", (aggregator_mapper,)) == "fastapi[extras=[all]]"

    def test_extras_come_after_the_version(self) -> None:
        assert (
            pep508_to_matchspec("fastapi[all]>=1.0", (aggregator_mapper,))
            == "fastapi >=1.0[extras=[all]]"
        )

    def test_multiple_extras_are_normalized_and_sorted(self) -> None:
        assert (
            pep508_to_matchspec("fastapi[Standard,ALL]", (aggregator_mapper,))
            == "fastapi[extras=[all,standard]]"
        )

    def test_extra_name_is_normalized(self) -> None:
        assert (
            pep508_to_matchspec("fastapi[Some_Extra.Name]", (aggregator_mapper,))
    ...
    def test_extra_name_at_exactly_64_characters_is_accepted(self) -> None:
        ...
        assert pep508_to_matchspec(entry, (aggregator_mapper,)) == f"fastapi[extras=[{'a' * 64}]]"

    def test_empty_extras_brackets_produce_no_extras_clause(self) -> None:
        assert pep508_to_matchspec("fastapi[]", (aggregator_mapper,)) == "fastapi"
    ...
    def test_duplicate_extras_after_normalization_are_deduplicated(self) -> None:
        ...
        """
        assert (
            pep508_to_matchspec("fastapi[Foo-Bar,foo_bar]", (aggregator_mapper,))
...
class TestMarkers:
    def test_virtual_package_marker_becomes_a_when_clause(self) -> None:
        ...
        assert pep508_to_matchspec(entry, (aggregator_mapper,)) == 'requests >=2.0.0[when="__win"]'

    def test_bare_name_with_marker_has_no_version_outside_the_brackets(self) -> None:
        entry = 'requests; sys_platform == "win32"'

        assert pep508_to_matchspec(entry, (aggregator_mapper,)) == 'requests[when="__win"]'
