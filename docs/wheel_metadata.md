...
* Items which are supposed to appear once (like Name), but actually appear multiple times should be treated as an error, not silently discarded
...
* Failure to parse one of the fields we are tracking corresponds an overall failure, not a silent default value. This is consistent with the Provides-Extra rule since that individual rule decides to not fail strings.
