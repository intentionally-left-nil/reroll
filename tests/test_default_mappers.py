...
# Each of grayskull_mapper, conda_lock_mapper, and parselmouth_mapper is
# fully tested in its own module; here we only need to confirm
# `default_mappers` calls all three and appends `aggregator_mapper` last,
# so the factories are stubbed out rather than built for real.


class TestDefaultMappers:
    def test_composes_grayskull_conda_lock_parselmouth_then_the_aggregator(
